# 0054-arm-kprobe-replace-patch_lock-to-raw-lock.patch分析

## 1 Patch原文

```patch
From 140c2dc25b8cb04fc7f0f623856d28cef0f3f0ff Mon Sep 17 00:00:00 2001
...
从OOPs信息中可以看出，这个BUG是在`linux-4.8.3-rt2`中发现和测试的，RT project没有`linux-4.8.3`的patch，我用`patch-4.8.2-rt2`对它打了patch，补丁完美打上，没有问题，所以我们获得了无限接近的`linux-4.8.3-rt2`，并以它为分析对象。

## 2 BUG分析

从Call Trace中看到，这里的问题和`linux-4.8.84-rt62中0089-usb-Use-_nort-in-giveback-function.patch的分析`中一样，也是在关中断的情况下睡眠了。
