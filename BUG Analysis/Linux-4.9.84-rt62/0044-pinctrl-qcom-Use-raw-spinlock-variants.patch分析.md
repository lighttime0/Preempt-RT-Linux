# 0044-pinctrl-qcom-Use-raw-spinlock-variants.patch分析

## 1 Patch原文

```patch
From 93ba90515af66da95dd0d3d8f773c0a9da2edd84 Mon Sep 17 00:00:00 2001
...
从上面的OOPs信息中可以看到，这个BUG是在`linux-4.4.9`中触发和测试的，RT project对`linux-4.4.9`只有`patch-4.4.9-rt17`一个补丁，所以我们就以它作为分析对象。

## 2 BUG分析

从Call Trace和BUG的定位行信息可以确定，BUG出现在下图中红框圈出来的那行。代码位于`linux-4.4.9/kernel/locking/rtmutex.c:1014`：

![rt_spin_lock_slowlock-linux-4.4.9-rt17](media/15237807275124/rt_spin_lock_slowlock-linux-4.4.9-rt17.png)

也就是说，代码执行到这里时，`rt_mutex_owner(lock) == self`这个条件没有满足，触发了BUG。
